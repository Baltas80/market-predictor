...
GDELT 1.0 daily files are named for the previous day's event-discovery date and posted the following morning. The pipeline therefore never treats `SQLDATE` or the 8-digit file date as availability. It assigns a conservative availability boundary of **06:00 America/New_York on the day after the file date**, with the IANA `America/New_York` timezone resolved using DST rules and then converted to UTC. Original article publication time remains unknown.
