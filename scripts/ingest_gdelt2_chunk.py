...
GDELT 2.0 is the primary event source for the new historical track. DATEADDED
is used as the point-in-time availability timestamp in UTC. Raw ZIP files are
processed in memory and discarded; only normalized daily checkpoints remain.
"""
...
def _canonical_date_added(value: object) -> str | None:
...
def _parse_date_added(series: pd.Series) -> pd.Series:
    canonical = series.map(_canonical_date_added).astype("string")
...
def fetch_chunk(start: str, end: str, checkpoint_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    while current <= end_date:
        ...
        day = deduplicate_events(pd.concat(day_frames, ignore_index=True))
