...
def test_recover_day_keeps_gdelt_publication_unknown_and_maps_pit_times(monkeypatch):
    raw = pd.DataFrame(
        {
            "global_event_id": ["g1"],
...
def test_recover_day_rejects_all_rows_dropped_by_pit_filter(monkeypatch):
    raw = pd.DataFrame(
        {
            "global_event_id": ["g1"],
            "sql_date": [pd.Timestamp("2015-02-19", tz="UTC")],
