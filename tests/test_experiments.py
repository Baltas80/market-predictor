...
def test_feature_ablation_uses_same_folds():
    data = _data()
    folds = [Fold(np.arange(0, 50), np.arange(50, 65)), Fold(np.arange(0, 65), np.arange(65, 80))]
    results = run_feature_ablation(data, folds, macro_features=["macro"], geopolitical_features=["geo"])
    assert [r.name for r in results] == ["technical", "technical_macro", "technical_macro_geopolitical"]
    assert all(len(r.predictions) == 30 for r in results)


def test_missing_feature_is_rejected():
    data = _data().drop(columns=["geo"])
    folds = [Fold(np.arange(0, 50), np.arange(50, 65))]
    with pytest.raises(ValueError, match="Missing columns"):
        run_feature_ablation(data, folds, macro_features=["macro"], geopolitical_features=["geo"])
