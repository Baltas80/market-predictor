...
from market_predictor.research_schema import EVENT_COLUMNS, deduplicate_events, normalize_event_sources, validate_event_frame, validate_market_frame
...
def _protocol(index_hash="idx"):
    return SharedExperimentProtocol(
        ...
        observations=20, prediction_index_hash=index_hash, dataset_hash="data", code_version="code",
    )
...
def test_event_schema_separates_event_publication_and_availability():
    frame = pd.DataFrame({
        "event_id": ["e1", "e2"], "event_time": ["2025-01-01T09:00:00Z", "2025-01-02T09:00:00Z"],
        "published_at": ["2025-01-01T10:00:00Z", "2025-01-02T10:00:00Z"],
        "available_at": ["2025-01-01T10:05:00Z", "2025-01-02T10:05:00Z"],
        "source_id": ["src", "src"], "category": ["war_conflict", "sanctions"], "severity": [0.8, 0.4],
        "country": ["US", "US"], "entity": ["x", "y"], "sector": [None, None],
        "duration_days": [0.0, 0.0], "media_intensity": [1.0, 2.0], "surprise": [0.2, 0.1],
    })
    validate_event_frame(frame)
    admitted = frame.loc[pd.to_datetime(frame.available_at, utc=True) <= "2025-01-01T10:06:00Z"]
...
def test_financial_matrix_has_fixed_3x2_grid_plus_random_baseline():
    ...
    assert result[["total_return", "max_drawdown", "sharpe", "turnover"]].notna().all().all()
    assert result["sortino"].notna().any()
...
def test_period_stability_reports_turnover_and_risk_metrics():
    ...
    backtest = pd.DataFrame({"strategy_return": [0.01, -0.005] * 5, "position_change": [1, 0] * 5}, index=idx)
    ...
    assert result.turnover.tolist() == [3.0, 2.0]


def test_market_schema_requires_timezone_and_finite_values():
    idx = pd.date_range("2025-01-01", periods=2, tz="UTC")
    frame = pd.DataFrame({"open": [1.0, 2.0], "high": [2.0, 3.0], "low": [1.0, 2.0], "close": [2.0, 3.0], "volume": [10.0, 20.0]}, index=idx)
