...
def _get(url: str, *, timeout: int = 60, params: dict | None = None) -> requests.Response:
    response = requests.get(url, params=params, timeout=timeout, headers={"User-Agent": "market-predictor/0.1 (research ingestion)"})
...
def load_fred_observations(series_id: str, api_key: str, *, realtime_start: str | None = None, realtime_end: str | None = None) -> pd.DataFrame:
    ...
    if frame.empty: return pd.DataFrame(columns=["date", "value", "realtime_start", "realtime_end"])
...
def align_fred_point_in_time(observations: pd.DataFrame, market_index: pd.DatetimeIndex, *, conservative_session_lag: int = 1) -> pd.DataFrame:
    ...
    for timestamp in market:
        ...
        if eligible.empty:
            rows.append({"date": timestamp, "value": pd.NA, "vintage": pd.NaT}); continue
...
def _gdelt_category(event_root_code: str, event_code: str, actor_text: str) -> str:
    root = str(event_root_code).zfill(2); code = str(event_code).zfill(4); actor_text = actor_text.lower()
    if code == "0163" or code.startswith("0163"): return "sanctions"
    if root == "14": return "social_unrest"
    if root in {"18", "19", "20"}: return "war_conflict" if root != "20" or not any(t in actor_text for t in ("terror", "militant", "extremist")) else "terrorism"
    if root in {"16", "17", "09", "10", "11", "12", "13", "15"}: return "political_crisis"
...
def load_gdelt_day(day: str | pd.Timestamp) -> pd.DataFrame:
    ...
    with zipfile.ZipFile(BytesIO(response.content)) as archive:
        ...
        if not members: raise ValueError(f"Empty GDELT archive for {date}")
...
def load_gdelt_range(start: str | pd.Timestamp, end: str | pd.Timestamp) -> pd.DataFrame:
    ...
    if end_date < start_date: raise ValueError("end must be on or after start")
    frames = []
    current = start_date
    while current <= end_date:
        frames.append(load_gdelt_day(current)); current += timedelta(days=1)
...
def _sec_category(title: str, description: str) -> str:
    ...
    if any(t in text for t in ("fraud", "accounting", "ponzi", "misstatement", "financial reporting")): return "financial_fraud"
    if any(t in text for t in ("insider trading", "bribery", "corruption", "scandal", "false statements")): return "corporate_scandal"
    return "regulation"


def load_sec_litigation_releases_rss() -> pd.DataFrame:
    response = _get(SEC_LITIGATION_RSS_URL, timeout=60)
    root = ET.fromstring(response.content); rows = []
    for item in root.findall(".//item"):
        title = (item.findtext("title") or "").strip(); description = (item.findtext("description") or "").strip()
        published = (item.findtext("pubDate") or "").strip(); link = (item.findtext("link") or "").strip(); guid = (item.findtext("guid") or link or title).strip()
        published_at = pd.to_datetime(published, utc=True, errors="coerce")
        if pd.isna(published_at): continue
...
def write_market_csv(frame: pd.DataFrame, path: str | Path) -> None:
    Path(path).parent.mkdir(parents=True, exist_ok=True); frame.to_csv(path, index=True, index_label="date")


def write_events_csv(frame: pd.DataFrame, path: str | Path) -> None:
    Path(path).parent.mkdir(parents=True, exist_ok=True); frame.to_csv(path, index=False)
