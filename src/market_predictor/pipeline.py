...
from .abc_protocol import common_walk_forward_folds
...
def _shared_final_lockbox_folds(data: pd.DataFrame, horizon: int, test_fraction: float) -> tuple[Fold, ...]:
    ...
    The returned tuple is the only fold input accepted by the A/B/C execution
    below. Feature variants cannot independently choose train/test boundaries.
    """
    n_rows = len(data)
    test_size = max(1, int(n_rows * test_fraction))
...
def run_final_lockbox_experiments(
    ...
    *,
    ...
    """Run A/B/C using one immutable fold sequence generated by the pipeline."""
    if horizon < 1:
        raise ValueError("horizon must be >= 1")
    if not 0 < test_fraction < 0.5:
        raise ValueError("test_fraction must be > 0 and < 0.5")
    ...
    # Critical invariant: folds are constructed once before any A/B/C model
    # execution and the same immutable sequence is passed to every variant.
